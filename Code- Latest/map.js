document.addEventListener("DOMContentLoaded", () => {
    var totalTime = 0;
    var truckTime = 0;
    var activeTrucks = 0; //max 1
    const sendTruckEvery = 50;//150; 
    const remPoints = 5;
    const truckInt = 450;
    const resDelay = 400;
    setInterval(() => {
    ...
    async function handleTruckArrival() {
      // remove the 5 closest points from that locality.
      console.log(currentHighlightedLocality);
      if (currentHighlightedLocality) {
        ...
        if (markers.length > 0) {
          ...
          for (const marker of markersToRemove) { // removing using delay
    ...
    socket.on("new-disaster", (report) => {
      ...
      function periodicTruckDispatch() {
        if (currentRouteCoordinates && truckTime > sendTruckEvery && activeTrucks<1){
